...
RAW_DIR = os.getenv("RAW_DIR", "/Users/eduardoalberto/LoadFile/raw")
STAGING_DIR = os.getenv("STAGING_DIR", "/Users/eduardoalberto/LoadFile/staging")
...
def process_generic(file_path):
    ...
    if ext in {".xlsx", ".xls", ".xlsm"}:
        ...
        with open(file_path, "r", encoding="utf-8", errors="ignore") as handle:
            df = pd.DataFrame({"conteudo": [handle.read()]})
...
def main():
    ...
    for arquivo in os.listdir(RAW_DIR):
        ...
        try:
            ...
            erros.append(f"{arquivo}: {error}")

    if erros:
        raise RuntimeError("Falhas no staging: " + "; ".join(erros))
